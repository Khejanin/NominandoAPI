                ...
                if(err) res.send(err);
                    ...
                    Player.findOne({_id:req.playerID},function (err,player) {
                        if(err) res.send(err);
                            ...
                            if(player){
                                payload.text = player.text;
                                payload.progressIndex = player.progressIndex;
                                res.send(payload);
                            }
                        }
                    });
...
exports.updateProgress = function(req,res){
    Player.findOne({_id:req.playerID},function (err,player) {
        if(err) res.send(err);
            ...
            if(player){
                player.progressIndex = req.body.progress;
                player.save();
                res.send(player.progress);
            }
        }
    });
};
...
exports.clear = function(req,res){
    Entity.deleteMany({user:req.playerID},function (err) {
        if(err) res.send(err);
            ...
            if(err) res.send(err);
            else Player.findOne({_id:req.playerID}, function(err,player){
				if(err) res.send(err);
				else{
				    if(player){
					    player.text = "";
				    ...
				    else res.send("Could not find player");
				}
			});
        });
    });
...
exports.uploadText = function(req,res){
    Player.findOne({_id:req.playerID}, function(err,player){
        if(err) res.send(err);
            ...
            if(player != null){
                player.text = req.body;
                player.save();
                res.send(player.text);
            }
            else{
